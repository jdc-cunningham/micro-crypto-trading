...
Awe damn this PM2 thing doesn't support optional chaining or my node version needs to be upgraded

Nah it's node I'm using 8 lol

You f it's still using 8 lmao

Doing first hosted test on the pi CRON 8:35 PM

it failed

Probably a dotenv path issue

I think it's a requirement path issue, I usually have this problem with CRON jobs I end up using the root path

eg. /full/root/path

Ugh it's also the file read/write path

Actually looks like I copied/pasted the wrong blank portfolio values

Nice went through, ran it at 8:57 PM

Oh man there's probably a timezone issue too on the RPi

Looks like the cron job is failing or at least the buying part is, no buys have gone through

No there are active buys but I think it's from me

yeah the file was modified 8:56

I'm waiting for the CRON log to write

Yeah the keys are blank, I updated the way they're defined

Starting to get data, had to fix the timestamp range check

<img src="./media/02-12-2022--starting-to-get-data.PNG" width="800"/>

Looks like it's failing to place a sell order

The numbers aren't correct, those are if the current order goes through

Particularly for the ones above 55

It's still trying to buy

LMAO I just noticed that 21 cents above

Oh yeah it double bought

10:40 PM
Finally the sell posted

It'll be interesting to see how soon this action is met, right now MDT is 0.0628 but the sell value is 0.06453

<img src="./media/02-12-2022--current-state.PNG" width="800"/>

This is what I have right now, I need to go make food

I need to make sure that it keeps working

Oh dayum! We making money! That's an "automated" trade baby

<img src="./media/02-12-2022--yeah-boiii.PNG" width="800"/>

200,000 more of these and I'll be rich!

This is gonna be cool waking up to this, that's like 11 hrs from now, I'm still up but yeah